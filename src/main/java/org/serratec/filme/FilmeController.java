...
public class FilmeController {
  ...
  @GetMapping("/{id}")
	...
	@DeleteMapping("/{id}")
	public ResponseEntity <Filme> excluirFilme (@PathVariable Long id) {
		 if(!repositorio.existsById(id)) {
		return ResponseEntity.notFound().build(); 
				 }
	...
	@PutMapping("/{id}")
	public ResponseEntity <Filme> alternarFilme(@PathVariable Long id ,@RequestBody  @Valid Filme filme) {
		 if(!repositorio.existsById(id)) {
		return ResponseEntity.notFound().build(); 
		 }
		 filme.setId(id);
